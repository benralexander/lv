 ...
 * to create one page of the molecular spreadsheet.
 *
 ...
 * we can ignore for the purposes of data structures and method signatures. Therefore in the text below I will always referred to experiments
 * as appearing within different columns and molecules appearing within different rows, with the understanding that in the actual presentation
 * to the user the orientation of these two axes may be reversed.
 */
class MssDefinition {
...
class  MssDataElement {
    ...
    AllTheOtherData allTheOtherData  // this element will contain everything else that the molecular spreadsheet might wish to display as a pop-up when the user clicks on an element
                                     ...
                                     // My presumption is that I can take whatever data structure is used for that page and incorporate it into this data structure.
                                     ...
                                     // had discussed serving this data directly from the backend, but I think it might make more sense to serve it from the Bard Web server
